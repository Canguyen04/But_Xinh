...
function removeAll(id, price) {
  if (confirm("Bạn chắc chắn muốn bỏ sản phẩm này")) {
    ...
    carts = carts.filter(
      (item) => item.id !== id || (item.id === id && item.price !== price)
    );
    localStorage.setItem("listCart", JSON.stringify(carts));
    renderCart();
  }
...
function renderCart() {
  ...
  arrSetUnique.forEach((item) => {
    ...
    $(".payment__tableBody").append(
      `
            <tr>
                <td><a onclick="removeAll('${item.id}',${
        item.price
        ...
        item.price
      })" class="fa fa-minus count_minus" aria-hidden="true"></i>
                        <span>${count}</span>
                        <i onclick="addItem('${item.id}',${
        item.price
      })" class="fa fa-plus count_plus" aria-hidden="true"></i>
                    </div>
                </td>

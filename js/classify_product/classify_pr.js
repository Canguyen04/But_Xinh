  ...
  $.getJSON("js/data.json").done((data) => {
    ...
    if (param === "bút bi") {
      dataProductsClassify = dataProductsClassify.filter(
        (item) => item.price.low.New >= PRICE_CAOCAP
      );
    } else if (param === "bút chì") {
      dataProductsClassify = dataProductsClassify.filter(
        (item) =>
          item.price.low.New >= PRICE_TRUNG && item.price.low.New < PRICE_CAOCAP
      );
    } else {
      dataProductsClassify = dataProductsClassify.filter(
        (item) => item.price.low.New < PRICE_TRUNG
      );
    }
